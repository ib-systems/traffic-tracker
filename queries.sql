...
SELECT ts, right(toString(port_uuid), 4) AS port, rx, tx
FROM port_samples
ORDER BY ts DESC
LIMIT 10;
...
FROM
(
    -- default 0 so the first sample counts in full, same as the view;
    -- max() per (port, ts) collapses same-second duplicates
    SELECT port_uuid, rx, lagInFrame(rx, 1, toUInt64(0)) OVER w AS p_rx
    FROM
    (
        SELECT port_uuid, ts, max(rx) AS rx
        FROM port_samples
        WHERE ts >= toDateTime('2026-06-01 00:00:00')
        GROUP BY port_uuid, ts
    )
